...
function updateComplexScore() {
  ...
  if (txt) {
    ...
    else { txt.textContent = 'Trois domaines affectés (' + totalChecked + ' items) — hautement compatible avec un TSPT complexe (s\'inspirant de la CIM-11).'; txt.style.color = '#ef4444'; }
  }
}
...
function generateSynthesis() {
  ...
  if (cxSynActive.length > 0) {
    ...
    if (cxSynActive.length === 1) {
      ...
      add("  Deux des trois domaines du TSPT complexe sont affectés. Ce tableau est compatible avec un TSPT complexe probable. Une évaluation psychiatrique spécialisée est fortement recommandée.");
    } else {
      add("  Les trois domaines sont atteints, configurant un tableau hautement compatible avec le TSPT complexe (probabilité clinique élevée). Prise en charge traumatologique spécialisée recommandée.");
    }
    var cxNotes = (document.getElementById('cx-notes')||{}).value||'';
    if (cxNotes) add('  Notes TSPT complexe : ' + cxNotes);
    add('');
  }
  ...
  // ── FACTEURS ASSOCIÉS — OBSERVATIONS ───────────────────────
  var aObs = g('a-obs');
  if (aObs) {
    add('');
    add('FACTEURS ASSOCIÉS — OBSERVATIONS COMPLÉMENTAIRES');
    add(sep);
    add('  ' + aObs);
    add('');
  }

  // ── TSPT DE L'ENFANT / ADO ─────────────────────────────────
  var childItems = [
    {id:'ch1', label:"Régressions comportementales (énurésie, langage, etc.)"},
    {id:'ch2', label:"Jeux traumatiques répétitifs"},
    {id:'ch3', label:"Cauchemars récurrents / terreurs nocturnes"},
    {id:'ch4', label:"Symptômes anxieux (séparation, phobies)"},
    {id:'ch5', label:"Troubles des apprentissages scolaires"},
    {id:'ch6', label:"Hypervigilance / agitation"},
    {id:'ch7', label:"Irritabilité / accès de colère disproportionnés"},
    {id:'ch8', label:"Symptômes dissociatifs"},
    {id:'ch9', label:"Retrait social ou apathie"},
    {id:'ch10',label:"Somatisations inexpliquées"}
  ];
  var childChecked = childItems.filter(function(it){ return ck(it.id); });
  var chAge       = g('ch-age');
  var chAgeTrauma = g('ch-age-trauma');
  var chContext   = g('ch-context');
  var chObsPed    = g('ch-obs-ped');

  if (childChecked.length > 0 || chContext || chObsPed) {
    add('');
    add("TSPT DE L'ENFANT ET DE L'ADOLESCENT");
    add(sep);
    if (chAge || chAgeTrauma) {
      var ageStr = chAge ? 'âge actuel : ' + chAge + ' ans' : '';
      var trStr  = chAgeTrauma ? 'âge au moment du traumatisme : ' + chAgeTrauma + ' ans' : '';
      add('  Données développementales : ' + [ageStr, trStr].filter(Boolean).join(' — ') + '.');
    }
    if (childChecked.length > 0) {
      var n = childChecked.length;
      var severity = n >= 7 ? 'sévère' : n >= 4 ? 'modéré à sévère' : 'partiel';
      add('');
      add('  Sur le plan clinique, ' + n + ' manifestation(s) pédiatrique(s) spécifique(s) du trauma sont identifiée(s), configurant un tableau de présentation ' + severity + ' :');
      childChecked.forEach(function(it){ add('    • ' + it.label + '.'); });
      add('');
      // Phrases cliniques selon nombre et nature
      var hasRegress   = ck('ch1');
      var hasPlay      = ck('ch2');
      var hasDissoc    = ck('ch8');
      var hasLearn     = ck('ch5');
      var hasSomat     = ck('ch10');

      var clinParts = [];
      if (hasRegress)
        clinParts.push("la présence de régressions comportementales évoque une réponse adaptative infantile face à une charge traumatique dépassant les ressources développementales disponibles");
      if (hasPlay)
        clinParts.push("les jeux traumatiques répétitifs constituent une expression symptomatique caractéristique, traduisant une tentative de maîtrise et d'élaboration de l'événement traumatique");
      if (hasDissoc)
        clinParts.push("les manifestations dissociatives observées sont cliniquement significatives et peuvent témoigner d'une exposition traumatique précoce ou d'une sévérité particulière de l'impact");
      if (hasLearn)
        clinParts.push("le retentissement sur les apprentissages scolaires constitue un indicateur fonctionnel objectif de l'impact du trauma sur les fonctions cognitives et attentionnelles");
      if (hasSomat)
        clinParts.push("les somatisations inexpliquées s'inscrivent dans un tableau de trauma non élaboré avec expression corporelle des affects non symbolisés");

      if (clinParts.length > 0) {
        add('  Sur le plan psychopathologique, ' + clinParts.join(' ; ') + '.');
        add('');
      }

      if (n >= 5) {
        add("  L'ensemble de ce tableau est cliniquement compatible avec un TSPT d'expression pédiatrique, selon les modalités développementales propres à l'âge de l'enfant. Une évaluation pédopsychiatrique spécialisée est recommandée à titre de probabilité diagnostique.");
      } else if (n >= 3) {
        add("  Ce tableau partiel (probabilité diagnostique modérée) suggère un impact traumatique significatif avec expression développementale. Un suivi pédopsychologique et une réévaluation à distance sont indiqués.");
      } else {
        add("  Ces éléments cliniques isolés nécessitent une surveillance longitudinale et une investigation approfondie du contexte développemental avant tout positionnement diagnostique.");
      }
    }
    if (chContext) { add(''); add('  Contexte développemental et familial : ' + chContext); }
    if (chObsPed)  { add('  Observations cliniques pédiatriques : ' + chObsPed); }
    add('');
  }

  // ── TSPT PUERPÉRAL ──────────────────────────────────────────
  var puerpContextItems = [
    {id:'p1', label:"Traumatisme lié à la grossesse"},
    {id:'p2', label:"Traumatisme lors de l'accouchement"},
    {id:'p3', label:"Trauma des suites de couches"},
    {id:'p4', label:"Hospitalisation néonatale"},
    {id:'p5', label:"Deuil périnatal"}
  ];
  var puerpSxItems = [
    {id:'p6',  label:"Ressenti de menace vitale (elle-même ou enfant)"},
    {id:'p7',  label:"Vécu de perte de contrôle"},
    {id:'p8',  label:"Flashbacks de l'accouchement / de l'événement"},
    {id:'p9',  label:"Évitement de tout rappel de l'\u00e9v\u00e9nement"},
    {id:'p10', label:"Hyperactivation émotionnelle"},
    {id:'p11', label:"Impact négatif sur le lien mère-enfant"},
    {id:'p12', label:"Difficultés d'allaitement en lien avec le trauma"}
  ];
  var puerpCtxChecked = puerpContextItems.filter(function(it){ return ck(it.id); });
  var puerpSxChecked  = puerpSxItems.filter(function(it){ return ck(it.id); });
  var puerpBond       = g('p-bond');
  var puerpObs        = g('p-obs');

  if (puerpCtxChecked.length > 0 || puerpSxChecked.length > 0 || puerpObs) {
    add('');
    add('TSPT PUERPÉRAL');
    add(sep);

    if (puerpCtxChecked.length > 0) {
      add('  Contexte périnatal identifié :');
      puerpCtxChecked.forEach(function(it){ add('    • ' + it.label + '.'); });
      add('');
    }

    if (puerpSxChecked.length > 0) {
      var ns = puerpSxChecked.length;
      var sxSev = ns >= 5 ? 'sévère' : ns >= 3 ? 'modéré' : 'partiel';
      add('  ' + ns + ' symptôme(s) spécifique(s) du TSPT puerpéral sont identifié(s), configurant un tableau clinique ' + sxSev + ' :');
      puerpSxChecked.forEach(function(it){ add('    • ' + it.label + '.'); });
      add('');

      var clinPuerp = [];
      if (ck('p8'))
        clinPuerp.push("la présence de flashbacks périnatalaux constitue un symptôme cardinal du TSPT puerpéral, avec reviviscence involontaire de l'événement obstétrical dans toute sa charge émotionnelle et sensorielle");
      if (ck('p6') && ck('p7'))
        clinPuerp.push("l'association entre ressenti de menace vitale et vécu de perte de contrôle lors de l'événement obstétrical représente un facteur de vulnérabilité majeur au développement d'un état de stress post-traumatique puerpéral");
      if (ck('p11'))
        clinPuerp.push("l'impact négatif sur le lien mère-enfant constitue une complication spécifique à ce tableau clinique, pouvant compromettre le développement précoce de l'attachement et justifiant une intervention thérapeutique prioritaire");
      if (ck('p5'))
        clinPuerp.push("la survenue d'un deuil périnatal confère à ce tableau une dimension traumatique complexe, associant le deuil de l'enfant ou de l'expérience de maternité au spectre traumatique de l'événement");
      if (ck('p9'))
        clinPuerp.push("les conduites d'évitement actif de tout rappel de l'événement obstétrical s'inscrivent dans la triade symptomatique du TSPT et peuvent altérer significativement les soins au nourrisson ou les décisions concernant une grossesse ultérieure");

      if (clinPuerp.length > 0) {
        add('  Sur le plan psychopathologique, ' + clinPuerp.join(' ; ') + '.');
        add('');
      }

      if (puerpBond && puerpBond !== '' && puerpBond !== "\u2014 Non \u00e9valu\u00e9 \u2014") {
        var bondPhrases = {
          "Absence d'impact identifié": "L'évaluation du lien mère-enfant ne met pas en évidence d'impact traumatique significatif à ce stade.",
          "Impact l\u00e9ger":    "Le lien mère-enfant présente un impact léger, pouvant nécessiter un soutien précoce à la parentalité.",
          "Impact mod\u00e9r\u00e9":   "Le lien mère-enfant est modérément perturbé, justifiant un accompagnement spécialisé en parentalité traumatique.",
          "Impact s\u00e9v\u00e8re":   "L'altération sévère du lien mère-enfant constitue une urgence clinique nécessitant une intervention pluridisciplinaire immédiate (pédopsychiatrie, psychologie périnatale, puériculture spécialisée)."
        };
        add('  Lien mère-enfant — ' + puerpBond + ' : ' + (bondPhrases[puerpBond] || ''));
        add('');
      }

      if (ns >= 4) {
        add("  Ce tableau est cliniquement compatible avec un TSPT puerpéral (probabilité diagnostique élevée). Une prise en charge spécialisée en psychiatrie périnatale est fortement recommandée, incluant l'évaluation du lien mère-enfant et le soutien à la parentalité.");
      } else if (ns >= 2) {
        add("  Ce tableau partiel est suggestif d'un impact traumatique périnatal (probabilité diagnostique modérée). Un suivi en psychologie périnatale et une réévaluation à 4–6 semaines sont indiqués.");
      } else {
        add("  Ces éléments isolés nécessitent une surveillance longitudinale dans le contexte du post-partum, sans permettre à ce stade un positionnement diagnostique ferme.");
      }
    }
    if (puerpObs) { add(''); add('  Observations cliniques puerpérales : ' + puerpObs); }
    add('');
  }
...
function generateConclusion() {
  ...
  if (cxCountC > 0) {
    var cxMsg = cxCountC === 1
      ...
      : cxCountC === 2
        ? 'Deux domaines affect\u00e9s. Tableau compatible avec un TSPT complexe probable (cliniquement \u00e9vocateur d\u2019un tableau CIM-11). \u00c9valuation sp\u00e9cialis\u00e9e indiqu\u00e9e.'
        : 'Trois domaines affect\u00e9s. Haute probabilit\u00e9 de TSPT complexe (s\u2019inspirant de la CIM-11). Prise en charge sp\u00e9cialis\u00e9e urgente recommand\u00e9e.';
    extras.push(cxMsg);
  }
...
function updateProgress() {
  ...
  updateNavBadge('child',     ['ch1','ch2','ch3','ch4','ch5','ch6','ch7','ch8','ch9','ch10','ch-age','ch-age-trauma','ch-context','ch-obs-ped']);
  updateNavBadge('puerp',     ['p1','p2','p3','p4','p5','p6','p7','p8','p9','p10','p11','p12','p-bond','p-obs']);
  ...
  if (_dissBadge) {
    ...
    _dissBadge.style.display = '';
    if (_checkedDissoc === 0) { _dissBadge.classList.remove('orange','green'); _dissBadge.classList.add('dark-red'); }
  }
}
...
function updateNavBadge(name, fieldIds) {
  ...
  badge.textContent = '';
  badge.style.display = '';  // always visible
  if (filled === 0) {
    badge.classList.add('dark-red');
  } else if (filled === 1) {
