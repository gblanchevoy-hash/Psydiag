...
function computeScore() {
  ...
  // ── Plancheurs cliniques sur l'indice ────────────────────────
  // P2 (idées passives) M ou P → surveillance renforcée minimum (index ≥ 21)
  // P3 (idées actives) ou P4 (scénario) M ou P → élevé minimum (index ≥ 41)
  // Alertes critiques → plancheurs supérieurs (inchangés)
  var lvlP2 = itemLevel('sP2');
  var lvlP3 = itemLevel('sP3');
  var lvlP4 = itemLevel('sP4');

  if (lvlP3 > 0 || lvlP4 > 0) {
    index = Math.max(index, 41);  // Élevé — intervention active requise
  } else if (lvlP2 > 0) {
    index = Math.max(index, 21);  // Modéré — surveillance renforcée
  }

  // Concordance avec les alertes critiques
